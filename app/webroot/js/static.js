...
function initFeed(){
	var cMgr = new H_CookieManager();
	var sidVal = cMgr.getCookie("set[feeds]");
	if(sidVal){
		var reArrVal = sidVal.split(':');		
		for(var i=0, len = reArrVal.length; i < len; i++ ) 
		{
			if(reArrVal[i] == '' ) continue;
			$('tr#sid_' + reArrVal[i] +' td:last-child').removeClass('foff').addClass('fon');	
		}
	}		
}
function initRssList(){
	var cMgr = new H_CookieManager();
	var str = cMgr.getCookie('set[rssList]');
	if(str){
		var arr = str.split('\t');
...
function setRssList(url){
	if(!url) return;
	var arr = [];
	var arrCopy = [];
	var str = '';
	var Reg = new RegExp('^'+url.encodeLatin()+'$','i');
	var rand = Math.floor(Math.random()*1000000000);
	$('#rssList > tbody').append('<tr id="r_'+ rand +'"><td style="width:3%;"><button><i class="icon-remove"></button></td><td>'+ url.htmlEscape() +'</td></tr>');
	$('#r_'+rand+' button').bind('click',function(event){
		//var  = $(this).parent('td').next('td').text();
		//console.log(str);
		var cMgr = new H_CookieManager();
		var str = cMgr.getCookie('set[rssList]');
		if(str){
			arr = str.split('\t');
			for(var i=0,len=arr.length;i<len;i++){
				if(!Reg.test(arr[i].encodeLatin())){
					arrCopy.push(arr[i]);
				}
			}
			str = arrCopy.join('\t');
			cMgr.setCookie('set[rssList]',str,3600 * 24 * 5000, '/');
		}
...
function checkSwitch(content,cookie,selector,classOn,classOff) { 
	...
	if(selector == 'feeds'){
		strId = sidC.match(/[0-9]+/);	
		reC = new RegExp(":"+strId+":");
		
		arrSid = sidVal.split(':');
		arrSidD = new Array();
		for(i=0,j=0,len=arrSid.length;i<len;i++)
		{
			if(arrSid[i] == '')
			{
				continue;
		...
		if(arrSidD.length >= limit && reC.test(sidVal))
		{
			sidVal = sidVal.replace(reC,"");
			cMgr.setCookie(cookie, sidVal, 3600 * 24 * 5000, '/');	
			sid = sid.id.match(/^sid_[0-9]+/i);
			$("tr#"+sid+" > td."+selector).toggleClass(classOn).toggleClass(classOff);
			return;
		}else if(arrSidD.length >= limit){
			alert('RSSのお気に入り登録は50個までです。');
			return;
		}
		sid = sid.id.match(/^sid_[0-9]+/i);
		$("tr#"+sid+" > td."+selector).toggleClass(classOn).toggleClass(classOff);
...
//--set Cookie 'feeds'
	...
	(
		function() 
		{ 
			checkSwitch(this,"set[feeds]",'feeds','fon','foff');
		}
...
//remove Cookie setting
	...
	$('#rmRss').click(function(){
		var flag = confirm('RSSの設定をリセットしますか？');
		if(flag){
			removeCookie('set[feeds]');
			reloadPage();
		}
	});
	...
	$('#rmRssList').click(function(){
		var flag = confirm('RSSリスト一覧（手動入力したURL）を削除しますか？');
		if(flag){
			removeCookie('set[rssList]');
			$('#rssList > tbody > tr').empty();
		}
	});
	//set DetailsSetting
	...
	$('#setDetail,#setDetailDemo').click(function(){
		...
		cMgr.setCookie("set[disprss]", disprss, 3600 * 24 * 5000, '/');
...
//display Table setting
    ...
    $('#liSmart').click(function(){
		var cMgr = new H_CookieManager();
		cMgr.setCookie("set[chTable]", '0.0.0.0.1.3.1', 3600 * 24 * 5000, '/');
    ...
    $('#liReset').click(function(){
		var cMgr = new H_CookieManager();
		cMgr.setCookie("set[chTable]", '1.1.1.1.1.0.0', 3600 * 24 * 5000, '/');
		...
		$('div.divHit').remove();		
		switchHit(0);
		$('#liBlogTail').removeClass('btn-info').addClass('btn-inverse').text('記事末尾にブログ名を非表示');				
		$('span.spBlog').remove();
	});
	//setting customRSS
	$('#getRss').click(function(){
		var limit = 50;
		var url = 'http://localhost/matomemo.net/matome/2ch/checkRss.php';
		var param = {val:encodeURIComponent($('#rssValue').val())};
		var rssUrl = $('#rssValue').val();
		var flag = false;
		var cMgr = new H_CookieManager();
		var str = cMgr.getCookie('set[rssList]');
		if(!str){
			cMgr.setCookie('set[rssList]', " ", 3600 * 24 * 5000, '/');
			str = cMgr.getCookie('set[rssList]');
		}
		var arr = ($.trim(str)).split('\t');
		var reg = RegExp('^http(:\/\/[-_.!~*\'()a-zA-Z0-9;\/?:\@&=+\$,%#]+)$');
		if(arr.length >= limit){
			alert('登録件数は'+limit+'件までです');
			return;
		}
		if(!reg.test(rssUrl)){
			alert('URLが正しくありません');
		...
		$('#rssList > tbody > tr > td').clone().unwrap().each(function(n){
			if(this.innerHTML === rssUrl){
				flag = true;
				alert('URLが重複しています。');
				$('#rssValue').val('');
				return;
...
function switchHit(sw){
	switch(sw){
		...
		break;
		case 1:
			$('#liHitHead').removeClass('btn-inverse').addClass('btn-info').text('記事先頭にHIT数を表示１');		
			$('table#latest tbody tr,table#people tbody tr,table#range tbody tr').each(function(n){			
				$('td.tdLink:eq('+n+') a').prepend("<div class='divHit'><span class='num'>" + this.children[4].innerHTML +"</span><span class='hit'>HIT</span></div>");		
			});
		break;
		case 2:
			$('#liHitHead').removeClass('btn-inverse').addClass('btn-info').text('記事先頭にHIT数を表示２');
			$('div.divHit').remove();
			$('table#latest tbody tr,table#people tbody tr,table#range tbody tr').each(function(n){			
				$('td.tdLink:eq('+n+') a').prepend("<div class='divHit' style='display:inline;'><span class='num'>" + this.children[4].innerHTML +"</span><span class='hit'>HIT</span></div>");		
			});
			$('div.divHit').css('display','inline');
		break;
		case 3:
			$('#liHitHead').removeClass('btn-inverse').addClass('btn-info').text('記事末尾にHIT数を表示');
			$('div.divHit').remove();
			$('table#latest tbody tr,table#people tbody tr,table#range tbody tr').each(function(n){	
				$('td.tdLink:eq('+n+') a').after("<div class='divHitTail' style='display:inline;'><span class='num'>" + this.children[4].innerHTML +"</span><span class='hit'>HIT</span></div>");		
		});
		break;
	}
}
function switchBlog(){
	...
	$('table#latest tbody tr,table#people tbody tr,table#range tbody tr').each(function(n){
		if($('td.tdLink:eq('+n+') div').hasClass('divHitTail')){
			$('td.tdLink:eq('+n+') div.divHitTail').after("<span class='spBlog'>" + this.children[6].children[0].innerHTML +"</span>");
		}else{
			$('td.tdLink:eq('+n+') .tdDiv').append("<span class='spBlog'>" + this.children[6].children[0].innerHTML +"</span>");			
		}
	});
	if($('*').is('#feed')){
...
//check chTable setting
//initCookie('set[chTable]','1.1.1.1.0.0.0');
